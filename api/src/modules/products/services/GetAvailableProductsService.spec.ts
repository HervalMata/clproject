...
import {GetAvailableProductsService} from "./GetAvailableProductsService";

let categoryRepositoryInMemory: CategoryRepositoryInMemory;
let createCategoryService: CreateCategoryService;
let productsRepositoryInMemory: ProductsRepositoryInMemory;
let createProductService: CreateProductService;
let getAvailableProductsService: GetAvailableProductsService;
...
describe('Get Available Products', () => {
    beforeEach(() => {
        ...
        getAvailableProductsService = new GetAvailableProductsService(productsRepositoryInMemory);
    });

    it('should be able to get available products', async () => {
        ...
        await expect(await getAvailableProductsService.execute({category_id})).toHaveLength(2);
    });

    it('should not be able to get non available products', async () => {
        await expect(await getAvailableProductsService.execute({})).toHaveLength(0);
    });
});