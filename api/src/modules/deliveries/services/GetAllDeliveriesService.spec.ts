...
import {GetAllDeliveriesService} from "./GetAllDeliveriesService";
...
let getAllDeliveriesService: GetAllDeliveriesService;
...
describe('Get All Deliveries', () => {
    beforeEach(() => {
        ...
        getAllDeliveriesService = new GetAllDeliveriesService(deliveriesRepositoryInMemory);
    });

    it('should be able to get all deliveries', async () => {
        ...
        const get_deliveries = await getAllDeliveriesService.execute();
        await expect(get_deliveries).toHaveLength(2);
    });
    it('should not be able to get all deliveries non existent', async () => {
        const get_deliveries = await getAllDeliveriesService.execute();
        await expect(get_deliveries).toHaveLength(0);
    });
});