...
describe('Get An Payment', () => {
    beforeEach(() => {
        ...
        getPaymentService = new GetPaymentService(paymentsRepositoryInMemory);
    });

    it('should be able to get an payment', async () => {
        ...
        const get_payments = await getPaymentService.execute({id: payment_id});
        await expect(get_payments).toBeInstanceOf(Payment);
    });
    it('should not be able to get an payment non existent', async () => {
        const id = "jnjfnbjginbgjio";
        const get_payments = await getPaymentService.execute({id});
        await expect(get_payments).not.toBeInstanceOf(Payment);
    });
});